...
# __Step 1__ Every participant begins by installing gitrules.
go install github.com/gitrules/gitrules/gitrules@latest
...
# - github.com/petar/gitrules.public (a public GitHub repo)
# - github.com/petar/gitrules.private (a private GitHub repo)
...
# __Step 3__ Every participant prepares their local gitrules client configuration.
...
cat <<EOF >> ~/.gitrules/config
{
     "cache_dir": "/Users/petar/.gitrules/cache",
...
# Also note that gitrules supports SSH as well as HTTPS urls for git repos.
...
cat <<EOF >> ~/.gitrules/config
{
     ...
     "auth" : {
          ...
          "https://github.com/petar/gitrules.public.git": { "access_token": "YOUR_ACCESS_TOKEN" },
          "https://github.com/petar/gitrules.private.git": { "access_token": "YOUR_ACCESS_TOKEN" }
     },
     "gov_public_url": "https://github.com/petar/community.public.git",
	"gov_public_branch": "gov",
	"gov_private_url": "https://github.com/petar/community.private.git",
	"gov_private_branch": "gov",
	"member_public_url": "https://github.com/petar/gitrules.public.git",
	"member_public_branch": "main",
	"member_private_url": "https://github.com/petar/gitrules.private.git",
	"member_private_branch": "main"
}
EOF

# By default gitrules looks for this config file in `~/.gitrules/config`, however you can specify a custom location
# using the `--config` flag, e.g. `gitrules --config /path/to/config.json ...`
...
gitrules user add --name petar --repo git@github.com:petar/gitrules.public.git --branch main
...
gitrules group add --name contributors

# ... and associate community users with a group.
gitrules member add --user petar --group contributors

# grant a few voting_credits tokens to user petar
# voting_credits can be spent on ballots.
gitrules balance add --user petar --key voting_credits --value 30.00
