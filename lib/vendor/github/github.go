...
import (
	...
	"github.com/gitrules/gitrules/lib/vendor"
...
func NewGitHubVendor(ctx context.Context, accessToken string) vendor.Vendor {
...
func NewGithubVendorWithClient(ctx context.Context, client *github.Client) vendor.Vendor {
	return &gitHubVendor{client: client}
}

func (x *gitHubVendor) CreateRepo(ctx context.Context, name string, owner string, private bool) (*vendor.Repository, error) {
	...
	if ok && errResp.Response.StatusCode == 422 {
		return nil, vendor.ErrRepoExists
	}
	if err != nil {
		return nil, err
	}
