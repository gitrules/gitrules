...
import (
	...
	"github.com/gitrules/gitrules/proto/motion/motionpolicies/waimea/concern"
...
func (x proposalPolicy) Open(
	...
	return nil, notice.Noticef(ctx,
		"Started managing this PR, using the Open-Source Management Protocol (Waimea), as GitRules proposal `%v` with initial __approval score__ of `%0.6f`."+
	...
	if !reflect.DeepEqual(propState, propStatePrev) {
		...
		if len(propState.EligibleConcerns) == 0 {
			...
			for _, ref := range propState.EligibleConcerns {
				...
				fmt.Fprintf(&w, "- %s, managed as GitRules concern `%v` with __priority score__ of `%0.6f`\n",
